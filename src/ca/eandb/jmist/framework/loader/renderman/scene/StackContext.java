 ...
 *
 */
public abstract class StackContext implements RenderManContext {
	
	private final Stack<RenderManContext> context = new Stack<RenderManContext>();
	...
	public void archiveRecord(RtToken type, String format, String... arg) {
		context.peek().archiveRecord(type, format, arg);
	}
	...
	public RtLightHandle areaLightSource(RtToken shadername,
		...
		return context.peek().areaLightSource(shadername, params);
	}
	...
	public void atmosphere(RtToken shadername, Map<RtToken, Object> params) {
		context.peek().atmosphere(shadername, params);
	}
	...
	public void attribute(RtToken name, Map<RtToken, Object> params) {
		context.peek().attribute(name, params);
	}
	...
	public void basis(RtBasis ubasis, int ustep, RtBasis vbasis, int vstep) {
		context.peek().basis(ubasis, ustep, vbasis, vstep);
	}
	...
	public void blobby(int nleaf, int ncode, int[] code, int ndoubles,
			...
			Map<RtToken, Object> params) {
		context.peek().blobby(nleaf, ncode, code, ndoubles, doubles, nstrings, strings, params);
	}
	...
	public void clipping(double near, double far) {
		context.peek().clipping(near, far);
	}
	...
	public void colorSamples(int n, double[] nRGB, double[] RGBn) {
		context.peek().colorSamples(n, nRGB, RGBn);
	}
	...
	public void concatTransform(RtMatrix transform) {
		context.peek().concatTransform(transform);
	}
	...
	public void cone(double height, double radius, double thetamax,
			Map<RtToken, Object> params) {
		context.peek().cone(height, radius, thetamax, params);
	}
	...
	public void coordSysTransform(RtToken name) {
		context.peek().coordSysTransform(name);
	}
	...
	public void coordinateSystem(RtToken name) {
		context.peek().coordinateSystem(name);
	}
	...
	public void cropWindow(double xmin, double xmax, double ymin, double ymax) {
		context.peek().cropWindow(xmin, xmax, ymin, ymax);
	}
	...
	public void curves(RtToken type, int ncurves, int[] nvertices,
		...
		context.peek().curves(type, ncurves, nvertices, wrap, params);
	}
	...
	public void cylinder(double radius, double zmin, double zmax,
		...
		context.peek().cylinder(radius, zmin, zmax, thetamax, params);
	}
	...
	public RtToken declare(String name, String declaration) {
		return context.peek().declare(name, declaration);
	}
	...
	public void depthOfField(double fstop, double focallength,
		...
		context.peek().depthOfField(fstop, focallength, focaldistance);
	}
	...
	public void detailRange(double minvisible, double lowertransition,
		...
		context.peek().detailRange(minvisible, lowertransition, uppertransition, maxvisible);
	}
	...
	public void disk(double height, double radius, double thetamax,
			Map<RtToken, Object> params) {
		context.peek().disk(height, radius, thetamax, params);
	}
	...
	public void displacement(RtToken shadername, Map<RtToken, Object> params) {
		context.peek().displacement(shadername, params);
	}

	/* (non-Javadoc)
	 * @see ca.eandb.jmist.framework.loader.renderman.RenderManContext#display(ca.eandb.jmist.framework.loader.renderman.RtToken, ca.eandb.jmist.framework.loader.renderman.RtToken, ca.eandb.jmist.framework.loader.renderman.RtToken, java.util.Map)
	 */
	@Override
	public void display(RtToken name, RtToken type, RtToken mode,
			Object... parmaterlist) {
		context.peek().display(name, type, mode, parmaterlist);
	}
	...
	public void errorHandler(RtErrorHandler handler) {
		context.peek().errorHandler(handler);
	}
	...
	public void exposure(double gain, double gamma) {
		context.peek().exposure(gain, gamma);
	}
	...
	public void exterior(RtToken shadername, Map<RtToken, Object> params) {
		context.peek().exterior(shadername, params);
	}
	...
	public void format(int xresolution, int yresolution, double pixelaspectratio) {
		context.peek().format(xresolution, yresolution, pixelaspectratio);
	}
	...
	public void frameAspectRatio(double frameaspectratio) {
		context.peek().frameAspectRatio(frameaspectratio);
	}
	...
	/* (non-Javadoc)
	 * @see ca.eandb.jmist.framework.loader.renderman.RenderManContext#procedural(java.lang.Object, ca.eandb.jmist.framework.loader.renderman.RtBound, ca.eandb.jmist.framework.loader.renderman.RtProcSubdivFunc, ca.eandb.jmist.framework.loader.renderman.RtProcFreeFunc)
	 */
	@Override
